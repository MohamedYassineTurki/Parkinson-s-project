...
function PairedOutcome({ children, comparison, doseSlot, medicationLabel, onHelp }: { children?: React.ReactNode; comparison: SavedPairComparison; doseSlot: number; medicationLabel: string; onHelp: () => void }) {
  const guidance = getComparisonGuidance(comparison.improvementPercent);
  return (
    <main className="min-h-[100dvh] bg-[#f4fafd] pb-28">
      ...
      <div className="mx-auto w-full max-w-[600px] px-5 pt-8">
        <section className="flex flex-col items-center text-center"><div className="flex size-24 items-center justify-center rounded-full bg-[#bbeacf] text-[#406b55]"><Link2 className="size-12" /></div><p className="mt-6 text-sm font-bold uppercase tracking-[0.1em] text-[#3c6751]">Dose {doseSlot + 1} comparison</p><h1 className="mt-3 text-[32px] font-bold leading-10 tracking-[-0.02em]">Your tests were linked</h1><p className="mt-3 text-lg font-semibold leading-7 text-[#161d1f]">{guidance.summary}</p></section>
        ...
        <section className="mt-4 grid grid-cols-2 gap-4"><MovementCard label="Before movement" value={comparison.beforePower} /><MovementCard label="After movement" value={comparison.afterPower} /><div className={`col-span-2 rounded-xl border p-5 ${guidance.tone === "higher" ? "border-[#f0bd8b] bg-[#ffdcbd]/40" : guidance.tone === "lower" ? "border-[#a2d1b6] bg-[#bbeacf]/35" : "border-[#bfc8c9] bg-[#eef5f7]"}`}><div className="flex items-center justify-between gap-4"><div><p className="font-bold">Difference</p><p className="mt-1 text-sm text-[#3f484a]">{guidance.label}</p></div><div className={`flex items-center gap-2 text-right text-xl font-bold ${guidance.tone === "higher" ? "text-[#93000a]" : "text-[#3c6751]"}`}><TrendingDown className={`size-6 shrink-0 ${guidance.tone === "higher" ? "rotate-180" : guidance.tone === "similar" ? "rotate-90" : ""}`} />{guidance.differenceLabel}</div></div></div></section>
        <section className={`mt-5 rounded-xl border p-5 ${guidance.tone === "higher" ? "border-[#f0bd8b] bg-white" : "border-[#dde4e6] bg-white"}`} aria-labelledby="comparison-next-step"><div className="flex items-start gap-3"><div className={`flex size-10 shrink-0 items-center justify-center rounded-full ${guidance.tone === "higher" ? "bg-[#ffdcbd] text-[#623f18]" : "bg-[#bbeacf] text-[#406b55]"}`}><Info className="size-5" /></div><div><h2 className="font-bold" id="comparison-next-step">What to do next</h2><p className="mt-2 text-base leading-7 text-[#3f484a]">{guidance.nextStep}</p><p className="mt-3 text-sm font-bold leading-6 text-[#004349]">{guidance.doctorAdvice}</p></div></div></section>
        <div className="mt-5 rounded-xl border border-[#dde4e6] bg-[#eef5f7] p-4 text-sm leading-6 text-[#3f484a]"><p><Info className="mr-1 inline size-4" /> This compares two phone measurements and is not a diagnosis.</p><p className="mt-2 font-semibold">Keep taking medication exactly as prescribed. Do not change the dose or timing because of this result.</p></div>

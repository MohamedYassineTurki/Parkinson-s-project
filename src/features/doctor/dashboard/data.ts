...
export async function getDoctorPatients(doctorProfileId: string) {
  ...
  for (const row of connected) {
    ...
    patients.push({ id: row.patientId, name: `${row.firstName} ${row.lastName}`, age: calculateAge(row.dateOfBirth), medicationName: medication ? `${medication.name} ${medication.dose}` : "No active medication", medicationSchedule: schedules.map((item) => item.scheduledLocalTime.slice(0, 5)), latestTestAt: latest?.testedAt ?? new Date(0).toISOString(), latestSeverityClass: latest?.severityClass ?? 0, latestTremorPower: latest?.tremorPower ?? 0, latestImprovementPercent: latestPair?.improvementPercent ?? 0, status: openAlert || summary.alertCandidate ? "review_suggested" : "stable", alertMessage: openAlert?.message, trend: history.pairs.map((pair) => ({ date: pair.testedAt, beforePower: pair.beforePower, afterPower: pair.afterPower, improvementPercent: pair.improvementPercent })) });
  }
  return patients;
}

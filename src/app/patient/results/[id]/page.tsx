...
export default async function PatientResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <DashboardShell description="Review the saved accelerometer metrics from one standardized test." navItems={[{ href: routes.patient.root, label: "Dashboard" }, { href: routes.patient.medications, label: "Medications" }, { href: routes.patient.test, label: "Run test" }, { href: routes.patient.history, label: "History" }, { href: routes.patient.onboarding, label: "Profile" }]} title="Test result">
